...
Those who are self-employed are roughly `{r} round((stroke_training |> filter(work_type == "Self-employed") |> summarize(p = mean(stroke == "Yes")) |> pull(p)) / (stroke |> filter(work_type != "Self-employed") |> summarize(p = mean(stroke == "Yes")) |> pull(p)), 0)` times as likely to suffer a stroke than those who work in other sectors, at around `{r} stroke_training |> filter(work_type == "Self-employed") |> summarize(p = mean(stroke == "Yes")) |> pull(p) |> scales::percent(accuracy = 1)` of cases.
...
Looking at the three models, XGBoost performed the best in terms of J-Index. Inspecting the confusion matrices, we can see that it outperformed both the kNN and Logistic Regression models in terms of sensitivity, successfully eliminating all False Negatives (`{r} xgb_cm |> filter(Truth == "Yes", Prediction == "No") |> pull(n)`). While the XGBoost model produced a higher number of False Positives (`{r} xgb_cm |> filter(Truth == "No", Prediction == "Yes") |> pull(n)`) compared to kNN (`{r} knn_cm |> filter(Truth == "No", Prediction == "Yes") |> pull(n)`) and Logistic Regression (`{r} log_cm |> filter(Truth == "No", Prediction == "Yes") |> pull(n)`), its perfect capture of positive stroke cases significantly boosted its overall performance. As the J-index metric weights both sensitivity and specificity equally, we will select the XGBoost model as our final candidate.
# Final Evaluation
