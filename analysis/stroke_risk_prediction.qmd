...
`r nrow(read_csv("data/processed/stroke_training.csv") + read_csv("data/processed/stroke_validation.csv") + read_csv("data/processed/stroke_testing.csv"))`
observations, we aim to identify patterns among these features and aid
...
![Pairwise relationships among selected stroke predictors](results/figures/01_ggpairs-plot.png){#fig-pairs}
...
![Percentage of stroke cases by gender](results/figures/02_stroke-by-gender.png){#fig-gender}
...
![Proportion of stroke cases by age group](results/figures/03_stroke-by-age.png){#fig-age}
...
![Heart Disease](results/figures/05_stroke-by-heart-disease.png){#fig-heartdisease}
:::

Conversely, there is no significant difference in likelihood for those
residing in urban versus rural areas (@fig-residence).

![Percentage of stroke cases by type of residence](results/figures/06_stroke-by-residence.png){#fig-residence}
...
::: {layout-ncol="2"}
![Average glucose level by stroke status](results/figures/07_glucose-by-stroke.png){#fig-glucose}
...
## Finalized Features
We moved forward with the features defined in our preprocessing script. We excluded `id`, `ever_married`, and `work_type` as they were determined to be too broad or uninformative.

##Model Comparison and Tuning
We tuned three distinct models. For the ***kNN model***, we performed a coarse sweep of neighbors, finding that performance plateaus around $k=150$ (@fig-coarse-tuning).
{#fig-coarse-tuning}

For the XGBoost model, we utilized Bayesian optimization and analyzed feature importance. Age and average glucose level emerged as the most critical features (@fig-xgb-vip).
{#fig-xgb-vip}

## Model Comparison and Tuning
For the **kNN model**, we performed a coarse sweep of neighbors, finding that performance plateaus around $k=150$ (@fig-coarse-tuning).

![J-Index vs # of Neighbours for kNN Model](results/figures/20_coarse-knn-k-sweep.png){#fig-coarse-tuning}

For the **XGBoost model**, we analyzed feature importance. Age and average glucose level emerged as the most critical features (@fig-xgb-vip).

![XGBoost Feature Importance](results/figures/21_xgboost-feature-importance.png){#fig-xgb-vip}

## Model Selection
We compared the three models on the validation set using J-index, sensitivity, and accuracy (@fig-metrics).

![Validation Set Metrics by Model](results/figures/22_validation-metrics-comparison.png){#fig-metrics}

Inspecting the confusion matrices in @fig-cms, we can evaluate how each model handled false positives and false negatives.

![Confusion Matrices on Validation Set](results/figures/23_validation-confusion-matrices.png){#fig-cms}

Based on the highest J-Index, the **`r val_metrics |> filter(.metric == "j_index") |> arrange(desc(.estimate)) |> slice(1) |> pull(model)`** model was selected as the best performing architecture.

# Final Evaluation
The best model was evaluated on the held-out test set. It achieved a final J-index of **`r test_metrics |> filter(.metric == "j_index") |> pull(.estimate) |> round(4)`**.

![Final Model Confusion Matrix on Test Set](results/figures/24_final-model-test-confusion.png){#fig-final-cm}
