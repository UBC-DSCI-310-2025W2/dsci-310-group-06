...
knn_cm   <- read_csv("../results/tables/05_knn-confusion-matrix.csv")
...
log_cm   <- read_csv("../results/tables/09_logreg-confusion-matrix.csv")
...
xgb_cm   <- read_csv("../results/tables/12_xgboost-confusion-matrix.csv")
...
trained and compared three classification models: k-nearest neighbors,
logistic regression with elastic-net regularization, and XGBoost. We used
an 80/10/10 train-validation-test split and 5-fold cross-validation for
hyperparameter tuning and Youden's J statistic was used as the primary
evaluation metric to account for severe class imbalance.
...
contributed little to model performance. Backward stepwise selection in
the logistic regression model retained age, average glucose level,
hypertension, heart disease, and smoking status. On the validation set,
logistic regression achieved the highest J-index of the three models and
was selected as the final candidate. Evaluated on the held-out test set,
it achieved a J-index of `r test_metrics |> filter(.metric == "j_index") |> pull(.estimate) |> round(4)`, correctly identifying most stroke cases while
producing a moderate number of false positives.

These findings suggest that a regularized logistic regression model can
meaningfully predict stroke risk from basic clinical features, though
the high false positive rate indicates room for improvement. Future work
should explore additional predictors related to lifestyle, genetics, and
medical history, as well as alternative strategies for handling class
imbalance.
...
factors contribute to the complexity of predicting the likelihood of a
...
Prediction Dataset, available via Kaggle. The dataset comprises `{r} nrow(stroke)`
observations and contains `{r} ncol(stroke)` different variables which make up our
clinical features for patients. These variables include gender, age,
hypertension, heart disease, marital status, type of work
...
values from the dataset, so that we continue analysis only with
fully-formed data.
...
Patients with hypertension (@fig-hypertension) are roughly twice as likely to have a stroke, which
is around `{r} stroke_training |> filter(hypertension == "Yes") |> summarize(prop = mean(stroke == "Yes")) |> pull(prop) |> scales::percent(accuracy = 0.1)` of the cases.

![Hypertension](../results/figures/04_stroke-by-hypertension.png){#fig-hypertension}

Those who have suffered heart disease (@fig-heartdisease) are roughly `{r} round((stroke_training |> filter(heart_disease == "Yes") |> summarize(p = mean(stroke == "Yes")) |> pull(p)) / (stroke_training |> filter(heart_disease == "No") |> summarize(p = mean(stroke == "Yes")) |> pull(p)), 0)` times as likely to have a stroke, which is around `{r} stroke_training |> filter(heart_disease == "Yes") |> summarize(p = mean(stroke == "Yes")) |> pull(p) |> scales::percent(accuracy = 1)` of the cases.
...
The percentages of cases are quite similar between the three groups.
...
Those who are self-employed are roughly `{r} round((stroke_training |> filter(work_type == "Self-employed") |> summarize(p = mean(stroke == "Yes")) |> pull(p)) / (stroke_training |> filter(work_type != "Self-employed") |> summarize(p = mean(stroke == "Yes")) |> pull(p)), 0)` times as likely to suffer a stroke than those who work in other sectors, at around `{r} stroke_training |> filter(work_type == "Self-employed") |> summarize(p = mean(stroke == "Yes")) |> pull(p) |> scales::percent(accuracy = 1)` of cases.
...
relation to strokes. By examining their associations with stroke, we
...
likely to clot which could cause a stroke.

![Stroke by age and
smoking](../results/figures/12_stroke-by-age-smoking.png)

All smoking groups had roughly equal numbers of stroke cases with the common ground being that with age, there was an average increase in percentage of strokes. The peak stroke percentage among the oldest age group was `{r} stroke_training |> filter(smoking_status == "Smokes") |> group_by(age = cut_interval(age, n = 5)) |> summarize(p = mean(stroke == "Yes")) |> pull(p) |> max() |> scales::percent(accuracy = 1)` for current smokers, `{r} stroke_training |> filter(smoking_status == "Never") |> group_by(age = cut_interval(age, n = 5)) |> summarize(p = mean(stroke == "Yes")) |> pull(p) |> max() |> scales::percent(accuracy = 1)` for those who never smoked, and `{r} stroke_training |> filter(smoking_status == "Formerly") |> group_by(age = cut_interval(age, n = 5)) |> summarize(p = mean(stroke == "Yes")) |> pull(p) |> max() |> scales::percent(accuracy = 1)` for former smokers. Interestingly, former smokers had higher percentages
...
In general, individuals without hypertension and heart disease have the lowest likelihood of having had a stroke, with a prevalence of `{r} stroke_training |> filter(hypertension == "No" & heart_disease == "No") |> summarize(p = mean(stroke == "Yes")) |> pull(p) |> scales::percent(accuracy = 0.1)`. However, those with both hypertension and heart disease exhibit the highest percentage, around `{r} stroke_training |> filter(hypertension == "Yes" & heart_disease == "Yes") |> summarize(p = mean(stroke == "Yes")) |> pull(p) |> scales::percent(accuracy = 1)`, of stroke cases. Participants with only heart disease have a lower likelihood, `{r} stroke_training |> filter(hypertension == "No" & heart_disease == "Yes") |> summarize(p = mean(stroke == "Yes")) |> pull(p) |> scales::percent(accuracy = 1)`, while those with only hypertension have an approximate `{r} stroke_training |> filter(hypertension == "Yes" & heart_disease == "No") |> summarize(p = mean(stroke == "Yes")) |> pull(p) |> scales::percent(accuracy = 1)` likelihood of having had a stroke.
...
25. However, this applies to only a minority of cases. Most cases where
...
We tuned the neighbors parameter via 5-fold cross-validation, performing a coarse sweep of neighbors from 0 to 200 stepping by 10. We will use Youden's J statistic to evaluate model performance as accuracy may improperly "mask" a poorly predictive model due to the class imbalance present. Youden's J statistic accounts for both sensitivity and specificity; a value of 1 indicates a perfect classifier, while a value of 0 indicates the model performs no better than random guessing.
...
We found that performance was best at k = `{r} knn_coarse |> filter(.metric == "j_index") |> arrange(desc(mean)) |> slice(1) |> pull(neighbors)` (@fig-coarse-tuning).

From @fig-coarse-tuning, we can see that model performance increases steadily as the number of neighbors increases, reaching a plateau around `{r} knn_coarse |> filter(.metric == "j_index") |> arrange(desc(mean)) |> slice(1) |> pull(neighbors)`. This suggests that in this dataset, a larger neighborhood is necessary to provide a stable decision boundary amidst the infrequent stroke cases. Based on this plateau, we will conduct a finer search around the `{r} knn_coarse |> filter(.metric == "j_index") |> arrange(desc(mean)) |> slice(1) |> pull(neighbors)`–200 neighbor range to land on a final value for our classifier.
...
It was found that k = `{r} knn_fine |> filter(.metric == "j_index") |> arrange(desc(mean)) |> slice(1) |> pull(neighbors)` results in the highest score on our training dataset. We will use this value to develop our final kNN model with neighbors = `{r} knn_fine |> filter(.metric == "j_index") |> arrange(desc(mean)) |> slice(1) |> pull(neighbors)`.
...
the AIC, resulting in only the terms listed below. We will use these
selected terms in the final model.
...
categorical predictors, we can see their importance is split
...
informative while we can drop categorical predictors that were
...
![XGBoost Confusion
Matrix](../results/figures/24_xgboost-confusion-matrix.png){#fig-xgb-cm}

## Model Selection

We compared the three models on the validation set using J-index,
sensitivity, and specificity (@fig-metrics).
...
Looking at the three models, Logistic Regression performed the best in terms of J-Index. Note that in the metrics tables above, because tidymodels treats the first factor level ("No") as the event, the reported "sensitivity" actually measures the true negative rate (specificity in the clinical sense) and "specificity" measures the true positive rate (sensitivity in the clinical sense). The J-index is unaffected by this distinction. Inspecting the confusion matrices, Logistic Regression correctly identified all actual stroke cases, with `{r} log_cm |> filter(Truth == "Yes", Prediction == "No") |> pull(n)` False Negatives (missed strokes). It produced `{r} log_cm |> filter(Truth == "No", Prediction == "Yes") |> pull(n)` False Positives, fewer than kNN (`{r} knn_cm |> filter(Truth == "No", Prediction == "Yes") |> pull(n)`). While XGBoost had the fewest False Positives (`{r} xgb_cm |> filter(Truth == "No", Prediction == "Yes") |> pull(n)`), it missed more positive stroke cases (`{r} xgb_cm |> filter(Truth == "Yes", Prediction == "No") |> pull(n)` False Negatives), resulting in a lower J-index. As J-index weights both sensitivity and specificity equally, we will select the Logistic Regression model as our final candidate.
...
of false positives is quite high, and the model also predicted some
...
them to live a healthier and more positive lifestyle. This may also
offer those who have risk factors that are genetic and not lifestyle
related, a better understanding of their risks. Additionally, this will also benefit
...
initiatives that better control smoking and be more specific in their
messages regarding the long term effects.

## What future questions could this lead to?

One key area for future exploration is the generalizability of the model
in real-world scenarios. This is specifically important in the event of
...
original sample it was trained with. However refined and well-trained the
model may be, it is crucial to consider the potential exceptions and
...
studies that focus on assessing the model's ability across various
