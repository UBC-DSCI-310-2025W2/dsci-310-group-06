...
Utilizing the Stroke Prediction Dataset, which comprises of
`r format(nrow(stroke), big.mark=",")`observations. We aim to identify patterns among these features and
...
### kNN Model
For the **kNN model**, we first performed a coarse sweep of neighbors from 0 to 200 stepping by 10. We found
that performance was best at k=`r knn_coarse |> filter(.metric == "j_index") |> arrange(desc(mean)) |> head(n=1) |> pull(neighbors)` (@fig-coarse-tuning).

![J-Index vs \# of Neighbours for kNN
Model](../results/figures/20_coarse-knn-k-sweep.png){#fig-coarse-tuning}

We then performed a finer sweep around the optimal k value, testing neighbors from `r knn_fine |> filter(.metric == "j_index") |> arrange(desc(mean)) |> head(n=1) |> pull(neighbors) - 10` to `r knn_fine |> filter(.metric == "j_index") |> arrange(desc(mean)) |> head(n=1) |> pull(neighbors) + 10` stepping by 1. The best performance was achieved at k=`r knn_fine |> filter(.metric == "j_index") |> arrange(desc(mean)) |> head(n=1) |> pull(neighbors)`.

Evaluating the final kNN model on the validation set yielded a J-index of `r knn_val |> filter(.metric == "j_index") |> pull(.estimate) |> round(4)`.


### Logistic Regression
For the logistic regression model, we performed backwards selection to identify the most informative predictors. Age, average glucose level, and the presence of pre-existing heart disease emerged as the strongest predictors of stroke. We then tuned the penalty and mixture parameters for the final model, with the best performance achieved at a penalty of `r logreg_params |> filter(.metric == "j_index") |> arrange(desc(mean)) |> head(n=1) |> pull(penalty)` and a mixture of `r logreg_params |> filter(.metric == "j_index") |> arrange(desc(mean)) |> head(n=1) |> pull(mixture)`. Evaluating the final logistic regression model on the validation set yielded a J-index of `r logreg_val |> filter(.metric == "j_index") |> pull(.estimate) |> round(4)`.
...
We then perform hyperparameter tuning on several key parameters such as number of trees, tree depth, learning rate, and minimum number of observations in a node. The best performance was achieved with `r xgb_params |> pull(trees)` trees, a tree depth of `r xgb_params |> pull(tree_depth)`, a learning rate of `r xgb_params |> pull(learn_rate) |> round(4)`, and a minimum number of observations in a node of `r xgb_params |> pull(min_n)`. Evaluating the final XGBoost model on the validation set yielded a J-index of `r xgb_val |> filter(.metric == "j_index") |> pull(.estimate) |> round(4)`.
...
[@cheung2021]. Considering muscle mass is inversely associated
with mortality [@lee2018], researchers may want to see to
